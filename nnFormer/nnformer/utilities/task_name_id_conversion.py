...
from nnformer.paths import nnFormer_raw_data, preprocessing_output_dir, nnFormer_cropped_data, network_training_output_dir
...
def convert_id_to_task_name(task_id: int):
    ...
    if len(unique_candidates) == 0:
        raise RuntimeError("Could not find a task with the ID %d. Make sure the requested task ID exists and that "
                           ...
                           "variables." %
                           (task_id,
                            os.environ.get('nnFormer_preprocessed') if os.environ.get('nnFormer_preprocessed') is not None else r'D:\walyn\ubuntu_copy\DATASET\nnFormer_preprocessed',
                            os.environ.get('RESULTS_FOLDER') if os.environ.get('RESULTS_FOLDER') is not None else r'D:\walyn\ubuntu_copy\DATASET\nnFormer_trained_models',
                            os.environ.get('nnFormer_raw_data_base') if os.environ.get('nnFormer_raw_data_base') is not None else r'D:\walyn\ubuntu_copy\DATASET\nnFormer_raw',
                            ))
