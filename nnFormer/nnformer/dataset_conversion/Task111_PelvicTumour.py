...
if __name__ == "__main__":
    train_dir = "/home/three03/walyn/Pelvic_Segmentation/data/train"
    test_dir = "/home/three03/walyn/Pelvic_Segmentation/data/test"


    output_folder = "/home/three03/walyn/DATASET/nnFormer_raw/nnFormer_raw_data/Task11_PelvicTumour"
    ...
    def load_save_test(args):
        data_file = args
        pat_id = data_file.split("/")[-1]
        pat_id = "test_" + pat_id.split(".")[-3]

        img_itk = sitk.ReadImage(data_file)
        sitk.WriteImage(img_itk, join(img_dir_te, pat_id + "_0000.nii.gz"))
        return pat_id

    nii_files_tr_data = subfiles(join(train_dir,'images'), suffix = "nii.gz")

    nii_files_tr_seg = subfiles(join(train_dir,'masks'), suffix = "nii.gz")

    nii_files_ts = subfiles(join(test_dir, 'img'), suffix = "nii.gz")
    ...
    json_dict['name'] = "PelvicTumour"
    json_dict['description'] = "PelvicTumour"
