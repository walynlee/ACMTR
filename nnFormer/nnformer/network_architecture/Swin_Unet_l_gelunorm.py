...
class swintransformer(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        patch_size=[2,4,4]
        self.model_down=SwinTransformer(pretrain_img_size=[96,128,128],window_size=4,embed_dim=embed_dim,patch_size=patch_size,depths=depths,num_heads=num_heads,in_chans=1)
        self.encoder=encoder(pretrain_img_size=[96,128,128],embed_dim=embed_dim,window_size=4,patch_size=patch_size,num_heads=[24,12,6],depths=[2,2,2])
   
        self.final=[]
        for i in range(len(depths)-1):
            self.final.append(final_patch_expanding(embed_dim*2**i,3,patch_size=patch_size))
