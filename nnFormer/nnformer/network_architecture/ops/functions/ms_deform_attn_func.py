...
def ms_deform_attn_core_pytorch_3D(value, value_spatial_shapes, sampling_locations, attention_weights):
    ...
    for lid_, (T_, H_, W_) in enumerate(value_spatial_shapes):
        ...
        print(value_l_.shape,sampling_grid_l_.shape, sampling_value_l_.shape)
        sampling_value_list.append(sampling_value_l_)
    attention_weights = attention_weights.transpose(1, 2).reshape(N_*M_, 1, Lq_, L_*P_)
    sample = torch.stack(sampling_value_list, dim=-2).flatten(-2)
  
    output = (sample * attention_weights).sum(-1).view(N_, M_*D_, Lq_)
