...
from nnformer.network_architecture.ACMTR import Generic_UNet
...
class nnFormerTrainerV2_PelvicTumour(nnFormerTrainer):
    """
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if not self.was_initialized:
            ...
            # we need to know the number of outputs of the network需要知道网络输出结果的数目
            net_numpool = len(self.net_num_pool_op_kernel_sizes)  # 4个

            # we give each output a weight which decreases exponentially (division by 2) as the resolution decreases
            # this gives higher resolution outputs more weight in the loss
            weights = np.array([1 / (2 ** i) for i in range(net_numpool)])  # weights: [1.   0.5  0.25]
    ...
    def initialize_network(self):
        ...
        # self.network = Att_UNet(1, 32, 3, 4)
        ...
        """
        self.network = NET(self.num_input_channels, self.base_num_features, self.num_classes,
                                       len(self.net_num_pool_op_kernel_sizes),
                                       self.conv_per_stage, 2, conv_op, norm_op, norm_op_kwargs, dropout_op,
                                       dropout_op_kwargs,
                                       net_nonlin, net_nonlin_kwargs, True, False, lambda x: x, InitWeights_He(1e-2),
        ...
        #print('I am not using the pre_train weight!!')
    ...
    def do_split(self):
        ...
        :return:
        """
        self.dataset_directory = r"D:\walyn\ubuntu_copy\DATASET\nnFormer_preprocessed\Task11_PelvicTumour"
        if self.fold == "all":
        ...
        # 输入fold 0/1/2/3/4
            ...
            if self.fold < len(splits):
                ...
                # print("tr_keys are :", tr_keys)
                # print("val_keys are :", val_keys)
                self.print_to_log_file("This split has %d training and %d validation cases."
                                       % (len(tr_keys), len(val_keys)))
                print("!!!!!!!!!!!!successful to split train dataset and valid dataset***************")
            else:
...
def get_default_configuration(network, task, network_trainer, plans_identifier=default_plans_identifier,
    ...
    if network == '2d':
        ...
        if task==1:
            ...
            plans_file = join(preprocessing_output_dir, task, plans_identifier + '_PelvicTumour'+"_plans_3D.pkl")
        else: 
            print("##################")
            plans_file = join(preprocessing_output_dir, task, plans_identifier + '_PelvicTumour'+"_plans_3D.pkl")
